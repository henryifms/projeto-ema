...
function totalPages(rawResponse: any, limit: number, currentLength: number) {
  if (rawResponse?.count) return Math.max(1, Math.ceil(rawResponse.count / limit));
  if (rawResponse?.total) return Math.max(1, Math.ceil(rawResponse.total / limit));
  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-slate-950">Tabela de leituras</h2>
          <p className="mt-1 text-sm text-slate-500">Tabela paginada e ordenável com os filtros aplicados na consulta.</p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-100 bg-white">
            {leituras.map((leitura) => (
              <tr key={leitura.id} className="hover:bg-slate-50">
                ...
                <td className="px-4 py-3 text-slate-700">{formatDate(leitura.data_leitura)}</td>
                <td className="px-4 py-3 text-slate-700">{leitura.temperatura}</td>
                <td className="px-4 py-3 text-slate-700">{leitura.umidade}</td>
                <td className="px-4 py-3 text-slate-700">{leitura.pressao_atmosferica}</td>
                <td className="px-4 py-3 text-slate-700">{leitura.velocidade_vento}</td>
                <td className="px-4 py-3 text-slate-700">{leitura.precipitacao}</td>
      ...
      <div className="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <p className="text-sm text-slate-500">Página {queryState.page} de {pages}</p>
        <div className="flex gap-3">
          <button
            disabled={queryState.page <= 1}
            onClick={() => onChangeQueryState((prev) => ({ ...prev, page: Math.max(1, prev.page - 1) }))}
          ...
          <button
            ...
            onClick={() => onChangeQueryState((prev) => ({ ...prev, page: prev.page + 1 }))}
